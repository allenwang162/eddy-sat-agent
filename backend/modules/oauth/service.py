...
from backend.shared.security import decrypt_json, encrypt_json, random_url_token, sha256_base64_url
...
class CodexCallbackHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            self.end_headers()
            self.wfile.write(f'<!doctype html><script>location.href="{env.APP_ORIGIN}/?codex_auth=success"</script><p>Codex connected.</p>'.encode("utf-8"))
        except Exception as exc:
            self.send_response(500)
            self.send_header("content-type", "text/html; charset=utf-8")
            self.end_headers()
            self.wfile.write(f"<p>{exc}</p><p><a href='{env.APP_ORIGIN}'>Return</a></p>".encode("utf-8"))
